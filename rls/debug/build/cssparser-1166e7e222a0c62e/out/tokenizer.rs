...
Tokenizer {
input : input , position : 0 , current_line_start_position : 0 , current_line_number : first_line_number , var_functions : SeenStatus :: DontCare , source_map_url : None , source_url : None ,
}
...
} # [ doc = " A position from the start of the input, counted in UTF-8 bytes." ] # [ derive ( PartialEq , Eq , PartialOrd , Ord , Debug , Clone , Copy ) ] pub struct SourcePosition ( pub ( crate ) usize ) ; # [ doc = " The line and column number for a given position within the input." ] # [ derive ( PartialEq , Eq , Debug , Clone , Copy ) ] pub struct SourceLocation {
...
let contents = & contents [ directive . len ( ) .. ] ; tokenizer . source_map_url = contents . split ( | c | {
c == ' ' || c == '\t' || c == '\x0C' || c == '\r' || c == '\n'
} ) . next ( )
} let directive = "# sourceURL=" ; let directive_old = "@ sourceURL=" ; if contents . starts_with ( directive ) || contents . starts_with ( directive_old ) {
let contents = & contents [ directive . len ( ) .. ] ; tokenizer . source_url = contents . split ( | c | {
c == ' ' || c == '\t' || c == '\x0C' || c == '\r' || c == '\n'
} ) . next ( )
}
